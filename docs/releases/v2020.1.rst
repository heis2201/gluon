...
With 2020.1 we drop support for the long deprecated compat14 version of
batman-adv. Communities still using this version should migrate to
batman-adv- using the scheduled domain switch.

IBSS wireless mesh removal
~~~~~~~~~~~~~~~~~~~~~~~~~~

Gluon 2020.1 drops support for the IBSS wireless protocol. Communities
...
Gluon now installs zram-swap by default on ``ar71xx`` devices with 8MB
of flash and 32M of RAM.
...
The status page now includes information about the nexthop prettyname,
mac address and interface towards the selected gateway when batman-adv
is used. It also shows the site and additionally the domain name if the
node is in a multidomain setup.

DECT button to enter config mode
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Many AVM devices don’t feature a separate RESET/WPS button, so starting
this release we support entering the config mode via DECT buttons.
...
We have added several new aliases for continued autoupdater
compatibility on the following devices:
...
-  The ``GLUON_WLAN_MESH`` variable can be dropped, as 802.11s is now
   the only supported wireless transport.
...
We now utilize continuous integration testing using Jenkins and thereby
